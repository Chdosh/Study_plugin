...
const runRealContract = process.env.RUN_AI_PROVIDER_CONTRACT === '1';
const contractTimeoutMs = Number.parseInt(process.env.AI_PROVIDER_CONTRACT_TIMEOUT_MS ?? '90000', 10);
const requestTimeoutMs = Number.parseInt(process.env.AI_PROVIDER_REQUEST_TIMEOUT_MS ?? '45000', 10);

const maybeDescribe = runRealContract ? describe : describe.skip;

function apiConfig() {
  const apiKey = process.env.STUDY_AI_API_KEY;
  const baseUrl = process.env.STUDY_AI_BASE_URL;
  const model = process.env.STUDY_AI_MODEL;
...
function expectConfiguredProvider(
  ...
  expect(apiKey, 'missing STUDY_AI_API_KEY').toBeTruthy();
  expect(baseUrl, 'missing STUDY_AI_BASE_URL').toBeTruthy();
  expect(model, 'missing STUDY_AI_MODEL').toBeTruthy();
}

function withTotalTimeout<T>(promise: Promise<T>, timeoutMs: number): Promise<T> {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error(`AI provider contract timed out after ${timeoutMs}ms`)), timeoutMs);
    promise.then(
...
function logContractStage(stage: string, started: number, errorType?: string): void {
  ...
  console.log(`[ai-provider-contract] stage=${stage} elapsedMs=${elapsedMs}${suffix}`);
}
...
maybeDescribe('configured AI provider real API contract', () => {
  ...
  it('validates short plan: returns rolling plan items with itemIndex 1+', async () => {
    expectConfiguredProvider(apiKey, baseUrl, model);
    ...
    try {
      ...
      const output = await withTotalTimeout(
        new AiClient().generateJson({
          ...
          user: [
            ...
            '输出 JSON 字段：weekFocus、items。每个单元包含 itemIndex、title、focus、tasks、expectedOutput、successCriteria。',
      ...
      expect(output.items[0].title.length).toBeGreaterThan(0);
      expect(output.items[0].tasks.length).toBeGreaterThan(0);
    } catch (error) {
      ...
      const output = await withTotalTimeout(
        new AiClient().generateJson({
          ...
          user: [
            ...
            '每日计划预留约 10%-15% 缓冲时间。',
            '每个 task 包含 title、objective、scope、estimatedMinutes、actions、deliverable、doneWhen、quickHint、evaluationMode。',
            ...
            '当前计划项：{"itemIndex":1,"title":"搭建环境与第一个组件","focus":"建立开发环境和组件概念","tasks":["搭建 React 项目","写第一个组件","理解 JSX 语法"],"expectedOutput":"可运行的 React 项目","successCriteria":"项目能启动并显示组件"}'
          ].join('\n')
        }),
        contractTimeoutMs
      );
    ...
    try {
      ...
      const output = await withTotalTimeout(
        new AiClient().generateJson({
          ...
          user: [
            '评估用户对当前学习步骤的提交。',
            '必须返回 JSON object，字段为 result、evidence、correctParts、misconceptions、missingRequirements、feedback、recommendedAction。',
            ...
            'result 为 passed 时 recommendedAction 只能是 advance 或 complete_task；其他 result 不能推荐 advance 或 complete_task。',
            '数组字段必须返回字符串数组。',
