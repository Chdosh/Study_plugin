...
删除了旧 `prepareNextLearningUnit`、错误混合 proposal/apply 的未使用草稿和 AppService 中的重复编排；归档前暂停 Session、评价后关闭 Guide 也已收回对应 Module Interface。验证基线：113 tests passed、6 skipped，typecheck 和 build 通过。下一步进入 Q2/P1.1 migration 事实源收敛。
...
P1 阶段完成验证：119 tests passed、6 skipped，typecheck 和 build 通过。项目达到 M1 内部稳定版，下一步进入 P2 提交—评价—推进事务闭环。
...
* 评价和状态推进仍是多段数据库写入；R1 已保证“提交先保存”和失败后可重试，完整 Evaluation/Decision/状态推进事务边界留给 T1 收敛。

## 推荐下一步

1. 收尾 R2：把待生成状态显式保存为 draft Guide，而不是只从 active ShortPlanDay + AI Review 推导。
2. 推进 R4：启动一致性审计和结构化冲突。
3. 进入 T1：统一核心 AI 操作事务边界，重点处理 Evaluation/Decision/状态推进的中断恢复。
4. 后续推进计划版本 UI、上下文硬预算、长期体验和旧 block/step 收敛。
