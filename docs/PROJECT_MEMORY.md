...
1. ✅ ~~落地 Phase 2 Step 2.3~~（已完成）
2. ✅ ~~落地 Phase 3 Step 3.1~~（已完成）
3. 落地 Phase 3 Step 3.2：完善冲突仲裁规则（时间限制以最后一次用户明确确认为准；基础知识取最近 3 次评估的交集）。当前只检测不仲裁。
4. 落地 Phase 4 Step 4.2：实现复盘→计划调整闭环（ReflectionAgent 输出含 shortPlanAdjustments schema，用户确认后修改 short_plan_days）。`plan_versions` 表只写无读。
5. 落地 Phase 5 Step 5.2-5.3：知识库项注入 evaluate_submission prompt + 复习触发规则（同一 misconception >=2 次自动加入下一天 daily guide）。knowledge_items 基础写入能力已有但未被 prompt 消费。
6. 落地 Phase 6 Step 6.1 + Phase 7-8：本地时区、测试补齐、知识库 UI + 文档闭环。
7. 用真实 DeepSeek API 验收一次完整分层计划生成。
