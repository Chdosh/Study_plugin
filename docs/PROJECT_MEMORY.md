...
2. 落地 Phase 3 Step 3.1：为 ContextBuilder 增加 operation 级别的上下文预算裁剪（按操作类型限制字段数量和长度，避免上下文随历史无限膨胀）。
3. 落地 Phase 3 Step 3.2：完善冲突仲裁规则（时间限制以最后一次用户明确确认为准；基础知识取最近 3 次评估的交集）。
4. 落地 Phase 4 Step 4.2：实现复盘→计划调整闭环（ReflectionAgent 输出含 shortPlanAdjustments schema，用户确认后修改 short_plan_days）。
5. 落地 Phase 5 Step 5.2-5.3：知识库项注入 evaluate_submission prompt + 复习触发规则（同一 misconception >=2 次自动加入下一天 daily guide）。
