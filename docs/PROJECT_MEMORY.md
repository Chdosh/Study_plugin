...
## 2026-07-12 产品审查整改批次 A

建立 `PRODUCT_REVIEW_REMEDIATION_2026-07-12.md`，按真实用户验收重新校准 Step 状态。完成 PR-01～PR-05：核心 AI 操作接入统一 ContextBuilder、最终 4000 token 预算、实际来源 ID、confirmed LearnerFact 注入边界、确定性冲突仲裁，以及 Review 摘要 pending/ready/failed 生命周期。新增 migration `202607120001_learner_fact_task_anchor`：global fact 使用空 goal anchor 跨目标共享，task fact 必须绑定 DailyGuideTask。Settings 可查看、确认、更新和撤回学习事实。验证：171 passed、6 skipped；typecheck、production build 和三类数据库迁移测试通过。下一批处理 T1/T2 与阶段推进复核。

## 2026-07-12 产品审查整改批次 B

完成 PR-06～PR-08。新增 migration `202607120002_submission_application_lifecycle`，将 AI 评价完成和业务状态应用拆为两个持久生命周期；pending/failed 可启动幂等重放，最终评价统一推进 Task、Focus Session、Guide 和 Runtime。Review 计划调整删除旧直写 IPC，只经 proposal/confirm。Roadmap 阶段耗尽改为 `ready_for_review`，删除单次评价直接完成阶段的旧路径，用户在 Review 明确确认后才激活下一阶段。验证：171 passed、6 skipped；typecheck、production build、空库/旧库/重复启动迁移测试通过。
...
### 2026-07-10 落地 R3 + K1 + K2：本地学习日 + 知识项来源证据 + 复习触发

**R3** — 本地学习日：`todayIso()` 从 UTC `toISOString().slice(0,10)` 改为本地时间 `YYYY-MM-DD`，避免跨日错误（`app-service.ts` + `app-service.test.ts`）。

**K1** — 知识项来源证据：新增 `knowledge_items` 表（含 `source_type`/`source_id` 追溯到 Submission/Evaluation/Task），bootstrap + migration `202607060009` 落地。`recordKnowledgeItems` 自动去重 + `occurrenceCount` 递增（+1 次出现）。

**K2** — 重复误区聚合与复习触发：`getReviewWorthyKnowledgeItems(goalId, minOccurrences=2)` 筛选 active 且累计 >=2 次的知识点。Daily Guide + Rolling Plan prompt 均注入复习队列段落，引导 AI 安排 5-10 分钟复习任务。

新增 prompt 级单测（`agent-prompts.test.ts`）覆盖：Daily Guide 含复习文本、Rolling Plan 含复习队列、无 review items 时省略。

验证：`npm test` 83 passed, 6 skipped；`npm run typecheck` 通过；`npm run build` 通过。

### 2026-07-10 落地 rolling plan + 复盘调整闭环 + 知识流全部贯通

**Rolling Plan 替代固定 3 天**：schema 去掉 dayIndex max(3) / days max(3) 硬限制（改为 max(5) / max(10) 单批上限、累计无上限）；prompt 去掉"前三天安排"表述。`generateRollingPlan` 实现：先复用当前阶段最早待学单元 → 耗尽后 AI 续写 + 新单元自动关联 `roadmap_stage_id`。`listAvailableShortPlanDaysForStage` 排除已被 guide 使用的 day。

**复盘调整闭环**：`reviewAgentOutputSchema` 新增 `planAdjustments` 数组；`applyReviewPlanAdjustments` 只改 pending + active stage 内的 unit，跳过 locked 单元。`confirmDailyGuide` 时锁定关联 short_plan_day（`locked = true`）。Review Page 展示调整建议 + 采纳按钮（已应用后禁用防重复）。旧 `pendingAdjustment` 卡片重命名为"即时补救建议"，明确仅影响路径推进。

**知识流贯穿**：submit 失败 → 自动写入 `knowledge_items`；后续 evaluate/daily guide/rolling plan prompt 均注入历史薄弱点；evaluate 时 AI 反馈可关联"你上次也犯过同类错误"。抽取 `getKnowledgeContextForGoal` 统一 3 处知识读取。`knowledge.listForGoal` IPC 通道 + Review Page 展示错题/薄弱点 UI。

验证：`npm test` 83 passed, 6 skipped；`npm run typecheck` 通过；`npm run build` 通过。

### 2026-07-10 落地 K3：知识状态重新验证更新
...
| 模块 | 文件 | 职责 |
|------|------|------|
| `PlanningModule` | `planning/planning.ts` | `prepareCurrentLearningDay()` + `advanceLearningDay()` + `generateRollingPlan()` + `closeCompletedLearningDay()` |
| `LearningRuntimeModule` | `runtime/runtime.ts` | `getSnapshot()` + Session Interface + `dispatch(command)` |
...
当前状态：Session 已迁入 LearningRuntimeModule；Daily Guide 生成和学习日推进已迁入 PlanningModule；评价知识流已迁入 LearnerContextModule。
...
**plan_versions 只读**：新增 `store.getPlanVersionsForGoal(goalId)` 查询目标的历史计划快照（通过 daily_guides → daily_plans → plan_versions 三表关联）。UI 层留待后续接入。

验证：`npm test` 90 passed, 6 skipped；`npm run typecheck` 通过；`npm run build` 通过。

### 2026-07-10 验证 R2 完成 + 审计集中修复

**R2 验证**：Daily Guide 生成失败恢复链路完整：失败时 ShortPlanDay 保持 `active`；重试时 `doPrepareCurrentLearningDay` 通过 `isRetry=true` 找到同一 day 继续生成；`generation_failed` 写入 `ai_reviews` 供审计。

**审计修复**：P2 双轨（复盘调整主路径唯一：Review Agent `planAdjustments` → `applyReviewPlanAdjustments`）；P3 DRY（`getKnowledgeContextForGoal` 统一 3 处知识读取）。
...
### 2026-07-10 落地 Phase 3 Step 3.1：ContextBuilder operation 级别上下文预算裁剪

为 `ContextBuilder.build()` 增加按操作类型的字段白名单 + 字段截断 + token 预算硬上限：

- **OPERATION_FIELD_WHITELIST**：每个操作只保留其 prompt 实际消费的字段。例如 `teach_step` 仅保留 `guideTask`/`guideAction`/`roadmapStage`，`evaluate_submission` 仅保留 `guideTask`/`latestSubmission`/`latestEvaluation`，`answer_step_question` 仅保留 `guideTask`/`guideAction`/`currentQuestionThread`
- **字段截断**：`goal.description` (300字)、`submission.content` (500字)、`questionMessages` (每条300字)，防止单字段无限膨胀
- **OPERATION_BUDGET_TOKENS = 4000**：序列化后估算 tokens（字符数/2），超出时按优先级依次裁剪 `roadmapStage` → `resolutionSummary` → `latestEvaluation` → `pendingAdjustment` → `goal.description`
- 估算方法用字符数/2（中文粗略估计），不做精确 tokenizer 依赖

修改范围：`src/main/services/context-builder.ts`（白名单 + 截断 + 预算裁剪）、`src/main/services/store.test.ts`（新增 3 个回归测试覆盖字段过滤和预算上限）。

验证：`npm run typecheck` 通过；`npm test`（90 passed, 6 skipped）通过；`npm run build` 通过。

### 2026-07-10 落地 Phase 2 Step 2.3：请求级幂等去重防护

为 `sendOnboardingMessage`、`askStepQuestion`、`submitLearningResult` 三个用户动作增加并发/双击去重保护。采用 **同步 Map 查找 + Promise 共享** 方案：用 `inFlight` Map 跟踪进行中的 Promise，用 `recentResults` Map 做 5 秒 TTL 结果缓存。dedupe 方法本身是同步的，确保两次并发调用在第一个 Promise 注册之后、微任务尚未切换之前就能命中已存在的 key。

关键决策：
- dedup key 仅使用内容本身（`onboarding:${trimmed}`），因为真实双击场景就是对同一操作的重复触发
- key 生成必须在 `dedupe` 调用前同步完成（无法 await），所以引入 StudyStore 内存缓存 `cachedActiveStepId` / `cachedActiveIntakeId`，在 `upsertRuntimeState` / `addGoalIntakeMessage` / intake 创建时更新
- 失败结果也会缓存（5 秒），确保双击失败后用户立刻重试不会产生第二次 AI 调用；5 秒后缓存过期允许真正重试

修改范围：`src/main/services/app-service.ts`（增加 dedupe 机制 + 三个方法重构为 public 同步入口 + private async 实现）、`src/main/services/store.ts`（增加 cachedActiveStepId / cachedActiveIntakeId + 三个 getter）、`src/main/services/app-service.test.ts`（新增 4 个回归测试覆盖并发去重和 TTL 过期）。

验证：`npm run typecheck` 通过；`npm test`（87 passed, 6 skipped）通过；`npm run build` 通过。

### 2026-07-10 落地 STEP_ROADMAP R1：提交评价可恢复

新增 `docs/STEP_ROADMAP.md`，按可靠闭环、AI 事务、上下文、知识证据、长期体验和兼容质量拆分后续 Step，并以代码和测试作为状态事实源。

修复新主任务流程的恢复断点：运行快照过去仍按旧 `step_id` 查询最新 Submission/Evaluation，导致写入 `daily_guide_action_id` 的失败提交在重启后不可见。现在查询同时兼容新 action 锚点和旧 step 锚点，Decision 通过 Evaluation 关联恢复。

新增提交评价重试能力：用户提交先以 `waiting` 保存，AI 失败后标记 `failed`；Study 页面和 AI 抽屉显示“提交已保存，评价未完成”，用户可复用原 Submission 重新评价。重试使用请求级 Promise 去重和持久评价锁，不创建重复 Submission；用户显式重试会清理进程异常退出遗留的同 Submission 锁。`SubmissionEvaluationResult` 同步收敛为新流程的 `nextAction` 语义。

新增两条 AppService 回归测试，覆盖“新 AppService 实例 + 遗留评价锁”的恢复，以及并发重试只产生一条 Evaluation。验证：`npm run typecheck`、相关 AppService 测试、`npm test`、`npm run build` 均通过。测试初始化仍有既有 migration duplicate column 跳过噪声。

### 2026-07-10 修复推进审查中的 R2 / R3 / K1-K3 / Review 反馈问题

按审查结果建立红测后完成以下修复：

- R2：`getTodayState()` 能识别未绑定 Guide 的 active ShortPlanDay 并返回 `generation_failed`；Today 增加“重新生成执行稿”入口；用户显式重试会清理旧进程遗留生成锁并继续原 ShortPlanDay。R2 仍保留 `in_progress`，因为待生成状态尚未显式保存为 draft Guide。
- R3：新增共享 `localDateIso()`，Main、Store 和 Renderer 的学习日全部使用本地日期，不再直接截取 UTC ISO 日期；增加 UTC 临界时间下生成下一学习单元的回归测试。
- K1/K2：新增迁移 `202607100001_knowledge_item_evidence` 和 `knowledge_item_evidence` 表，每次误区出现可关联 Submission、Evaluation 和 DailyGuideTask；知识键按技术词或中文核心词规范化，模型措辞变化不再轻易拆成不同知识项。
- K3：知识状态重新验证固定使用评价前捕获的 `guideTask`，不再在状态机推进后误用下一任务的完成标准。
- Review：计划调整 UI 根据实际更新数量显示“已应用”或“没有可应用项”；调用失败保持可重试并沿用全局错误提示。

新增回归测试覆盖 R2 重启 + 遗留锁恢复、本地日期临界点、跨任务 K3、同义知识聚合和多来源证据。验证：`git diff --check`、`npm run typecheck`、`npm test`、`npm run build` 均通过；既有 migration duplicate column 跳过噪声仍在。

### 2026-07-11 固化 Runtime / Planning 主链

三轮清理后，Action/Task/Session 运行时命令统一通过 `LearningRuntimeModule`；Daily Guide 生成、失败恢复、学习日关闭、Review 容错和下一学习单元推进统一通过 `PlanningModule`。Action 结束不再绕过 Submission/Evaluation，Task skip 不再按掌握处理，“结束本次学习”保留可恢复 Session。

删除了旧 `prepareNextLearningUnit`、错误混合 proposal/apply 的未使用草稿和 AppService 中的重复编排；归档前暂停 Session、评价后关闭 Guide 也已收回对应 Module Interface。验证基线：113 tests passed、6 skipped，typecheck 和 build 通过。

### 2026-07-11 完成 Q2 / P1.1 migration 事实源收敛

修复 bootstrap 先创建最新列、历史 migration 再重复 `ALTER TABLE` 的双重事实源。全新库只预登记 bootstrap 已覆盖的 6 个加列 migration，其余 migration 继续真实执行；已有库只执行缺失 migration，移除 duplicate-column / already-exists 吞错分支。

新增迁移矩阵覆盖全新空库、已升级库重复启动、缺最后一版 migration 的旧库升级，并校验用户目标保留、全部 migration 记录和 `PRAGMA foreign_key_check`。验证：117 tests passed、6 skipped，typecheck 和 build 通过；duplicate-column 日志已消失。

### 2026-07-11 完成 R2 / P1.2 显式 draft Guide 恢复

Daily Guide 生成调用前先为目标 ShortPlanDay 事务创建空任务的 draft Guide。AI 失败或进程退出后，启动状态直接读取该 draft；显式重试复用其 plan/guide 并原位写入任务和 Action，成功后切换为 active。回归测试确认失败前后 Guide ID 不变且只产生一个有效 Guide。

### 2026-07-11 完成 R4 / P1.3 启动一致性审计

应用创建窗口前执行 runtime audit，并通过 AppService 缓存把启动结果交给 Renderer。唯一可推导的 Goal/Stage/Task/Action/Session 状态自动修复；多 Session、跨 Goal、关闭 Guide 等歧义只报告。工作区显示恢复或冲突提示，用户可重新检查或保留数据稍后处理。

P1 阶段完成验证：119 tests passed、6 skipped，typecheck 和 build 通过。项目达到 M1 内部稳定版。

### 2026-07-11 M2 收敛（P2→P6 全部完成）

按 EXECUTION_PLAN_TO_HIGH_USABILITY_2026-07-11.md 的 P2→P6 全部落地：

**P2 提交—评价—推进事务闭环**
- `saveEvaluationAndDecision` 三包写入包入 `db.transaction()` 原子化
- 新增 `recoverPendingEvaluationProgress` 启动时恢复方法（evaluationStatus=completed 但 task 未完成）
- `AppService.initialize()` 末尾调用恢复，幂等重放
- +4 测试覆盖事务原子性/崩溃恢复/幂等重放/跳过未通过

**P3.1 计划 Proposal/Apply + T3 版本**
- 新增 4 个 IPC 通道（data:getPlanVersions / createPlanProposal / confirmPlanProposal / rejectPlanProposal）
- Store 新增 `createProposal` / `confirmProposal`（幂等、跳过锁定单元）/ `rejectProposal`
- PlanningModule 新增 `proposePlanChange` / `confirmPlanChange`
- ReviewPage 新增"计划变更历史"卡片
- +5 测试

**P3.2 LearnerContext 深仲裁 + 事实分层**
- Schema 新增 `learner_facts` 表（scope: task/goal/global, source: user_stated/inferred/confirmed）
- `LearnerContextModule` 完整重构（proposeFact/confirmFact/getFact/listFactsForGoal/deleteFact）
- `context-builder.ts` build() 内部读取 goal facts 注入 `context.learnerFacts`
- `teachCurrentStep`/`answerStepQuestion`/`evaluateSubmission` 改用 `this.modules.context.build()`（消除 contextBuilder 直接依赖）
- 删除 `AppService.contextBuilder` 字段
- +9 测试（store + context module + context-builder）

**P3.3 LearningBranch 完整实现**
- `question_threads` 表新增 `kind` 列（question/debug/practice）
- `LearningBranchModule` 完整实现（open/append/close[4种策略]/resolve/promote）
- close 策略：close / extract_knowledge / propose_fact / promote_task
- `askStepQuestion` 改为通过 branch 创建问题线程
- AiDrawer 增加分支收束策略选择 UI
- +8 测试

**P4 UI 交互基线**
- 新建 `shared/progress.ts` 统一进度算法（doneActions/totalActions）
- 新建 `renderer/domain/command-policy.ts`（computeCommandPolicy）
- Today/Study/Review 三处按钮统一按 policy 渲染
- 局部反馈：操作后按钮附近 3s 自动消失 message
- 全局通知栏仅保留 bootError 错误级别
- +7 测试

**P5+P6 知识复习 + 学习风格 + Session 锚点收敛**
- ReviewPage 知识项复习徽章（occurrenceCount>=2）+ 标记按钮
- Settings 教学风格选择（简洁/详细/代码优先）
- Token 成本统计 `getTokenCostStats`（按 operation/date 聚合 ai_reviews）
- Session 锚点收敛：`startSession(blockId→taskId)` / `skipBlock` 标记 deprecated / `getActiveSession` 移除 legacyPlanBlockId 回退
- +2 测试

当前基线：160 tests passed、6 skipped（16 test files），typecheck 和 production build 通过。
下一步：Q1 Session 锚点收敛 → Q3（14 天/30 Session 真实连续验收，opt-in）。

### 2026-07-12 主流程深度审查 + 6 项缺陷修复

按审查报告从 P0→P2 顺序修复全部已发现缺陷：

**P0 — confirmFact 覆盖已有事实内容**
- `LearnerContextModule.confirmFact` 传入 `value: ''` 导致已有事实被清空，改为先读取已有 fact 保留 value
- `context.ts` + `context.test.ts`（新增 2 测试覆盖保留 value 和不存在的场景）

**P1 — getDaySnapshot questionTopics 永远为空**
- `getDaySnapshot` 使用 legacy `stepId` 查询 `question_threads`，新流程该字段为 null
- 改为通过 task actions 的 `dailyGuideActionId` 查询，`store.ts` + `store.test.ts`

**P1 — pendingEvaluations 永远为空数组**
- `listTodayGuide` 写死 `pendingEvaluations: []`，Today 页"待处理"评价计数永远为 0
- 新增 `store.getPendingEvaluationIdsForGoal(goalId)` 查询当前 active guide 下所有 waiting 状态的 submission
- `store.ts` + `app-service.ts` + `store.test.ts`

**P2 — promote_task 缺少用户确认断点**
- `close()` 中 promote_task 改为 throw，强制用户通过 `promote()` 显式确认
- `promote()` 完善为 `createTaskFromBranch` + `resolveQuestion` 完整流程
- 扩展 IPC/preload/shared types/App.tsx/AiDrawer 类型签名，`branch.test.ts`（新增 3 测试）

**P2 — 5 处 dead code 清理**
- 删除 `__old_open_question_placeholder`、`createStepForActiveExecutionTask`、`getOrCreateActiveStepForAction`、`createLearningStep`、`listRecentStepSummaries`、`updateCurrentStepFromTeaching`、`getLearningStep`

**P2 — Roadmap 阶段推进时机优化**
- 新增 `store.advanceRoadmapStageIfAllGuidesComplete`：阶段下所有 short plan day 都已激活且全部 closed 时推进阶段
- 集成到 `generateRollingPlan` 流程中（而非主任务提交路径，避免过早推进）
- `planning.ts`（PlanningStore type 扩展）+ `store.test.ts`

验证：`npm test` 160 passed, 6 skipped；`npm run typecheck` 通过；`npm run build` 通过。

### 2026-07-11 P0 文档同步与代码审查

文档同步：
- `STEP_ROADMAP.md`：C1/C2/C3/U1-U4/Q2 从 pending 改为 completed；当前基线/风险/推荐执行顺序更新
- `PROJECT_MEMORY.md`：删除重复的"落地 U3"和"迁入 LearnerContextModule"条目；移除验证段落中的冗余"下一步"；更新"当前主流程"前三天→滚动计划；"当前风险"和"推荐下一步"对齐 M1 状态
- `PROJECT_OVERVIEW.md`：测试计数 62→101；架构图加入 modules 容器；模块目录增加 5 个模块；"前 3 天"→"滚动计划"；新增 §11 主链调用映射

代码审查结论：
- 模块边界：Planning/Runtime 全部走模块 ✅；LearnerContextModule.build 被 AppService 的 contextBuilder 直接调用（已知，留 P3.2 修正）
- 遗留路径：skipBlock / getAccumulatedSeconds 仍使用 block API（留 Q1 修正）
- 无 AppService 重复编排；无 prepareNextLearningUnit / 旧 terminateLearning 残留

## 当前风险

* 真实 DeepSeek 完整主流程仍需人工验收一次（Q3）。
* 旧 plan/step/block 数据结构仍作为 session 锚点存在；skipBlock 已标记 deprecated，Q1 剩余清理可在后续版本推进。
* 幂等去重依赖内存缓存的 activeStepId —— 应用重启后缓存丢失，但不影响正确性（最多退化为无去重）。
* 知识流（recordKnowledgeItems）失败后无重试机制（P2 范围外，留后续优化）。
* `AgentReductionStrategy` 应急收紧策略已落地但未经真实高压场景验证。
* promote_task 已有主进程确认断点，但 renderer UI 尚未提供 promote 入口（AiDrawer 组件仍只有 close / extract_knowledge 两个策略按钮）。

## 推荐下一步

1. **Q1**：Session 锚点收敛 —— 移除 study_sessions.blockId/taskItemsId 列（需先确认新 session 锚点稳定）。
2. **Q3 / P7**：14 天/30 Session 真实 DeepSeek 连续验收（opt-in）。
3. **知识流重试**：recordKnowledgeItems 失败时写入 pending 队列，下次评价时补录。
4. **promote_task UI 入口**：AiDrawer 组件增加提升为任务的策略选项 + 确认弹窗。
5. **UI 视觉微调**：P4 已建基线，后续可做信息密度和视觉层级优化。
